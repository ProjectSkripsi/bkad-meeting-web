...
const DataTweet = ({
  ...
  const onChange = (e, data) => {
    ...
    if (name === 'type' && value === 'paripurna') {
      const partId = data && data.map((item) => item.value);
      console.log('selectData', partId);
      setState((prevState) => ({ ...prevState, [name]: value, units: partId }));
    } else {
  ...
  const onSaveNoTulen = (id, eventResult, next) => {
    ...
    axios
      .patch(
        ...
        { text: eventResult, participant: selectedOption?.key },
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
