...
const TodoListItem = ({
  ...
  const [isBidang, setIsBidang] = useState(false);
  ...
  const onViewodal = (type, data, tanggal) => {
    if (type === 'count') {
      setIsBidang(true);
      setDataMember(data);
  ...
  const endDatePlusOne = (date) => {
    const dates = moment(date).add(1, 'days');
    const isBefore = moment(dates).isAfter(Date.now());
    return isBefore;
  };
          ...
          <div className='d-flex flex-row mb-3 justify-content-between'>
            <div className='flex-grow-1'>
              <p className='font-weight-medium mb-0 '>
                <CardTitle>
                  {item.name} {'     '} {endDatePlusOne(item.end)}
                  <Badge
                    color={
          ...
          <div className='row'>
            ...
            <div className='col-md-4 text-right'>
              ...
              <Button
                ...
                disabled={item.status === 'active' && !activeCheck(item.end)}
              >
                Download QR Code
              </Button>
            </div>
